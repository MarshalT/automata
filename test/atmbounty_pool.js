...
// 格式化数据为消息
function formatMessage1(data) {
    if (!data?.player?.data || !data?.state) return null;
    return `🎮 Automata奖池状态更新
- 奖励池: ${data.state.bounty_pool}
    ...
    while (true) {
        ...
        if (data) {
            ...
            if (currentBountyPool !== previousBountyPool) {
                ...
                const message = formatMessage1(data);
                if (message) {
                    console.log('准备发送的消息:\n', message);
